 ...
 */
export default function Tooltip({ text, position = 'top', children, delay = 0 }: TooltipProps) {
    ...
    >
      ...
      {isVisible && (
        <div
          ...
          role="tooltip"
          aria-label={text}
          className={`absolute z-50 px-md py-sm bg-charcoal-900 dark:bg-charcoal-700 text-white dark:text-charcoal-50 rounded-md text-sm whitespace-nowrap pointer-events-none ${positionClasses[position]}`}
        >
          {text}
          <div
            className={`absolute w-0 h-0 border-4 border-transparent ${
              position === 'top' ? 'border-t-charcoal-900 top-full left-1/2 -translate-x-1/2' :
