...
export default function CorporateSidebar() {
    ...
    return (_jsx("aside", { className: "w-56 border-r h-full p-4", children: _jsx("nav", { className: "space-y-2", children: items.map(i => (_jsx(NavLink, { to: i.to, className: ({ isActive }) => `block px-3 py-2 rounded ${isActive ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-gray-700 hover:bg-gray-50'}`, children: i.label }, i.to))) }) }));
}
