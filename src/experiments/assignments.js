...
export function assignUserToVariant(flag, ctx) {
    if (!flag.enabled || flag.killSwitch)
        return flag.defaultVariant ?? "control";
    if (ctx.forceVariant)
        return ctx.forceVariant;
    if (flag.force && ctx.userId && flag.force[ctx.userId])
        return flag.force[ctx.userId];
    ...
    if (flag.type === "boolean") {
        const variantKey = chooseVariant(flag.key, ctx.userId, flag.variants ?? [{ key: "control", rolloutPercentage: 100 }], flag.salt);
