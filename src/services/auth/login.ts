...
import { ServiceResponse, AuthUser, CompanyUser } from '../../types';
 ...
 */
export async function loginWithMasterId(masterIdOrBareUsername: string, password: string): Promise<ServiceResponse<LoginResult>> {
  try {
    if (!masterIdOrBareUsername || !password) return { success: false, error: 'Missing credentials', code: 'INVALID' };
    ...
    const companyRows: CompanyUser[] = (companies || []).map((c: any) => ({ ...c, company_alias: (c.company_alias || '').toLowerCase() }));
    ...
    if (roles.length === 0) primaryRole = null;
    else if (roles.every(isContractorRole)) primaryRole = 'contractor';
    else if (roles.some(isCorporateRole)) primaryRole = 'corporate';
