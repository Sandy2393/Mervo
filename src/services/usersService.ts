...
import { CompanyUser, ServiceResponse } from '../types';

class UsersService {
  /**
   * List users in a company
   */
  async listCompanyUsers(companyId: string): Promise<ServiceResponse<CompanyUser[]>> {
    try {
   ...
   */
  async getCompanyUserById(companyUserId: string): Promise<ServiceResponse<CompanyUser>> {
    try {
      const { data, error } = await supabase
        .from('company_users')
        .select('*')
  ...
  async updateCompanyUser(
    ...
    updates: Partial<Omit<CompanyUser, 'id' | 'company_id' | 'user_id' | 'created_at'>>
  ): Promise<ServiceResponse<CompanyUser>> {
    try {
      const { data, error } = await supabase
        .from('company_users')
   ...
   */
  async deactivateCompanyUser(companyUserId: string): Promise<ServiceResponse<CompanyUser>> {
    try {
      const { data, error } = await supabase
        .from('company_users')
   ...
   */
  async listContractors(companyId: string): Promise<ServiceResponse<CompanyUser[]>> {
    try {
      const { data, error } = await supabase
        .from('company_users')
        .select('*')
