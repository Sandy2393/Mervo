export async function listWorkforce(companyId) {
    const res = await fetch(`/api/companies/${companyId}/workforce`);
    if (!res.ok)
        throw new Error("Failed to load workforce");
    return res.json();
}
...
export async function importCsvPreview(companyId, csv) {
...
export async function importCsvCommit(companyId, csv) {
    const text = await csv.text();
    ...
    if (!res.ok)
        throw new Error("CSV commit failed");
    return res.json();
}
