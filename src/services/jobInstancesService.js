 ...
 */
export async function createJobInstance(company_id, jobId, scheduled_for, meta) {
    try {
 ...
 */
export async function assignJobInstance(company_id, instanceId, company_user_alias, actorId) {
    try {
        ...
        if (actorId) {
            ...
            if ((perms['jobs'] || 'none') !== 'edit' && cu.role_level < 90) {
 ...
 */
export async function listInstancesForUser(user_id, filters) {
    try {
        let query = supabase
            .from('job_instances')
            .select(`
 ...
 */
export async function updateInstanceStatus(company_id, instanceId, status, actorId) {
    try {
        ...
        if (actorId) {
            ...
            const actorRoleLevel = cu?.role_level || 0;
            if (actorRoleLevel < 0) {
 ...
 */
export async function listInstancesFiltered(company_id, filters) {
    try {
        let query = supabase
            .from('job_instances')
            .select(`
 ...
 */
export async function createRecurringInstances(jobId, company_id, recurrenceRule, numOccurrences = 90) {
    try {
 ...
 */
function generateScheduledTimes(rules, numOccurrences) {
    const times = [];
