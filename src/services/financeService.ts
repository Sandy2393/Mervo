...
export interface RevenueData {
  ...
  job_count: number;
  average_per_job_cents: number;
  byJob: RevenueByJob[];
  breakdownByDay: { date: string; total_cents: number }[];
}
...
): Promise<ServiceResponse<RevenueData>> {
  try {
    ...
    const byJob: RevenueByJob[] = (instances || []).map((inst: any) => {
...
export async function getPendingPayments(
  ...
  try {
    ...
    // Group by contractor
    const paymentMap = new Map<string, { amount_cents: number; count: number }>();
    ...
    const payments: PaymentData[] = Array.from(paymentMap.entries()).map(
      ([alias, { amount_cents, count }]) => ({
...
export async function calculateProfit(
  company_id: string,
  job_instance_id: string
): Promise<ServiceResponse<{ revenue_cents: number; contractorCost_cents: number; platformFees_cents: number; profit_cents: number; marginPercent: number }>> {
  try {
    ...
    if (error || !instance) return { success: false, error: 'Instance not found', code: 'NOT_FOUND' };
 ...
 */
export async function forecastRevenue(company_id: string, nextNDays: number = 7): Promise<ServiceResponse<{ dailyCents: { date: string; amount_cents: number }[] }>> {
  try {
