...
import { Job, JobInstance, ServiceResponse } from '../types';
...
class JobsService {
  ...
  async createJob(
    companyId: string,
    job: Omit<Job, 'id' | 'created_at' | 'company_id'> & { created_by: string },
    actorId?: string
    ...
    try {
      ...
      if (actorId) {
        ...
        if ((perms['jobs'] || 'none') !== 'edit' && ((cu as any)?.role_level || 0) < 90) {
          throw new PermissionDeniedError('create_job_permission');
        }
      }

      const validation = validateJobPayload(job as any);
      if (!validation.valid) {
   ...
   */
  async getJobById(companyId: string, jobId: string): Promise<ServiceResponse<Job>> {
    try {
      const { data, error } = await supabase
        .from('jobs')
        .select('*')
  ...
  async listJobsByCompany(
    companyId: string,
    filters?: { status?: string; publish?: boolean }
  ): Promise<ServiceResponse<Job[]>> {
    try {
      let query = supabase
        .from('jobs')
  ...
  async updateJob(
    companyId: string,
    jobId: string,
    updates: Partial<Omit<Job, 'id' | 'company_id' | 'created_at'>>,
    actorId?: string
  ): Promise<ServiceResponse<Job>> {
    try {
  ...
  async createJobInstance(
    companyId: string,
    jobInstanceData: Omit<JobInstance, 'id' | 'created_at'>
   ...
   */
  async updateJobInstance(
    companyId: string,
    jobInstanceId: string,
    updates: Partial<Omit<JobInstance, 'id' | 'company_id' | 'job_id' | 'created_at'>>,
    actorId?: string
  ): Promise<ServiceResponse<JobInstance>> {
    try {
