...
import { AuthUser, CompanyUser, ServiceResponse } from '../types';
...
class AuthService {
   ...
   */
  async login(master_alias: string, password: string): Promise<ServiceResponse<LoginResponse>> {
    try {
      ...
      if (companyUsers && Array.isArray(companyUsers)) {
        companyUsers.forEach((cu: any) => {
          if (cu.company_alias) cu.company_alias = (cu.company_alias as string).toLowerCase();
        });
      }
   ...
   */
  async createMasterAccount(accountId: string, password: string, metadata?: Record<string, any>): Promise<ServiceResponse<AuthUser>> {
    try {
   ...
   */
  async getUserCompanies(userId: string): Promise<ServiceResponse<CompanyUser[]>> {
    try {
   ...
   */
  async verifyCompanyAccess(userId: string, companyId: string): Promise<ServiceResponse<CompanyUser | null>> {
    try {
      const { data, error } = await supabase
        .from('company_users')
        .select('*')
