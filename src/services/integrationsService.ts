...
export async function listInstalled(companyId: string) {
  const res = await fetch(`${base}/companies/${companyId}/connectors`);
  if (!res.ok) throw new Error("Failed to list installed connectors");
  return res.json();
}

export async function installConnector(companyId: string, connectorId: string, config: any) {
  const res = await fetch(`${base}/companies/${companyId}/connectors`, {
    method: "POST",
...
export async function sendTestEvent(subscriptionId: string, samplePayload: any) {
  const res = await fetch(`${base}/webhooks/${subscriptionId}/test`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(samplePayload),
  });
  if (!res.ok) throw new Error("Test event failed");
  return res.json();
}

export async function listDeliveries(subscriptionId: string) {
...
export function renderTemplatePreview(template: string, data: any) {
