...
import { checkLinkCycle, isSamePerson, AccountMetadata } from './auth-utils';

class AccountLinkService {
  /**
   * List linked accounts for a master account
   */
  async listLinkedAccounts(masterAccountId: string): Promise<ServiceResponse<any[]>> {
    try {
  ...
  async linkAccounts(
    ...
    try {
      ...
      if (verifyIdentity) {
        const p: AccountMetadata = { email: primary.email, phone: primary.phone };
        const t: AccountMetadata = { email: toLink.email, phone: toLink.phone };
        if (!isSamePerson(p, t)) {
      ...
      if (existingLinks && checkLinkCycle(existingLinks.map((l: any) => ({ primary_id: l.primary_account_id, linked_id: l.linked_account_id })), primaryAccountId, toLinkAccountId)) {
   ...
   */
  async unlinkAccount(linkId: string): Promise<ServiceResponse<null>> {
    try {
