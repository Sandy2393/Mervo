...
const htmlEscape = (value) => value
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
const renderTemplate = (template, data, escapeHtml = true) => template.replace(/{{\s*([\w.]+)\s*}}/g, (_match, token) => {
    const value = token.split(".").reduce((acc, key) => (acc ? acc[key] : undefined), data);
...
export async function fetchNotificationLog(companyId) {
