...
class TimesheetService {
     ...
     */
    async clockIn(jobInstanceId, userId, geoData) {
        try {
     ...
     */
    async clockOut(jobInstanceId, userId, geoData) {
        try {
     ...
     */
    async getTimesheetByJobInstance(jobInstanceId) {
        try {
     ...
     */
    async listUserTimesheets(userId, limit = 50) {
        try {
            const { data, error } = await supabase
                .from('timesheets')
                .select('*')
     ...
     */
    async getEarningsSummary(userId, startDate, endDate) {
        try {
            const { data, error } = await supabase
                .from('timesheets')
            ...
            const totalDurationSeconds = (data || []).reduce((sum, ts) => sum + (ts.duration_seconds || 0), 0);
