...
class CompanyUserService {
     ...
     */
    async createCompanyAlias(companyId, accountId, companyAlias, role, permissions = {}) {
        try {
            // Normalize the alias
            ...
            const existingRoles = (accountRoles || []).map((r) => r.role);
    ...
    async listCompanyUsers(companyId, filters) {
        try {
            let q = supabase.from('company_users').select('*').eq('company_id', companyId);
            if (filters?.role)
