...
import { Company, CompanyOwner, ServiceResponse } from '../types';
...
class CompanyService {
   ...
   */
  async getCompanyById(companyId: string): Promise<ServiceResponse<Company>> {
    try {
   ...
   */
  async listUserCompanies(): Promise<ServiceResponse<Company[]>> {
    try {
      const { data, error } = await supabase
        .from('companies')
        .select('*')
   ...
   * Get company owners
   */
  async getCompanyOwners(companyId: string): Promise<ServiceResponse<CompanyOwner[]>> {
    try {
      const { data, error } = await supabase
        .from('company_owners')
        .select('*')
   ...
   */
  async addCompanyOwner(companyId: string, userId: string, isPrimary = false): Promise<ServiceResponse<CompanyOwner>> {
    try {
      const { data, error } = await supabase
        .from('company_owners')
   ...
   */
  async removeCompanyOwner(ownerId: string): Promise<ServiceResponse<CompanyOwner>> {
    try {
      const { data, error } = await supabase
        .from('company_owners')
        .delete()
   ...
   */
  async suspendCompany(companyId: string): Promise<ServiceResponse<Company>> {
    try {
      const { data, error } = await supabase
        .from('companies')
