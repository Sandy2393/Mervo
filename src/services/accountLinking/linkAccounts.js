...
export async function requestLink(primaryUserId, secondaryUserId) {
    try {
        if (!primaryUserId || !secondaryUserId)
            return { success: false, error: 'Missing ids', code: 'INVALID' };
        if (primaryUserId === secondaryUserId)
            return { success: false, error: 'Cannot link same account', code: 'INVALID' };
...
export async function confirmLink(verificationCode) {
    try {
        if (!verificationCode)
            return { success: false, error: 'Missing code', code: 'INVALID' };
        const { data: req, error: fetchErr } = await supabase
            ...
            .maybeSingle();
        if (fetchErr)
            return { success: false, error: fetchErr.message, code: 'FETCH_ERROR' };
        if (!req)
            return { success: false, error: 'Invalid or expired code', code: 'NOT_FOUND' };
        // check expiry
...
export async function mergeMasterAccounts(primaryId, secondaryId) {
    try {
        ...
        if (migrateErr)
            return { success: false, error: migrateErr.message, code: 'MIGRATION_ERROR' };
