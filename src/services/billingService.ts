...
import { ServiceResponse, Invoice } from '../types';
 ...
 */
export async function createInvoice(company_id: string, payload: Partial<Invoice>): Promise<ServiceResponse<Invoice>> {
  try {
...
export async function listInvoices(company_id: string, filters?: { status?: string; start?: string; end?: string }): Promise<ServiceResponse<Invoice[]>> {
  try {
    let query: any = supabase.from('invoices').select('*').eq('company_id', company_id);
...
export async function getInvoiceById(company_id: string, invoiceId: string): Promise<ServiceResponse<Invoice>> {
  try {
    const { data, error } = await supabase
      .from('invoices')
      .select('*')
...
export async function markInvoicePaid(company_id: string, invoiceId: string, paymentDetails?: any): Promise<ServiceResponse<Invoice>> {
  try {
    ...
    if (fetchErr || !existing) return { success: false, error: 'Invoice not found', code: 'NOT_FOUND' };
...
export async function generateInvoiceCSV(company_id: string, start?: string, end?: string): Promise<ServiceResponse<{ csv: string }>> {
  try {
    // TODO: For large exports, implement server-side streaming/export.
    let q: any = supabase.from('invoices').select('*').eq('company_id', company_id).order('created_at', { ascending: false });
    if (start) q = q.gte('period_start', start);
