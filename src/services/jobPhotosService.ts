...
import { JobPhoto, ServiceResponse } from '../types';
...
class JobPhotosService {
   ...
   */
  async getJobPhotos(jobInstanceId: string): Promise<ServiceResponse<JobPhoto[]>> {
    try {
      const { data, error } = await supabase
        .from('job_photos')
        .select('*')
   ...
   */
  async getPhotoUrl(storagePath: string, expiresIn: number = 3600): Promise<ServiceResponse<string>> {
    try {
   ...
   * Delete photo
   */
  async deletePhoto(photoId: string, storagePath: string): Promise<ServiceResponse<null>> {
    try {
      // Delete from storage
