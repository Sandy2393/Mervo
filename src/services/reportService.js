 ...
 */
export async function uploadPhotos(instanceId, userId, photos) {
    try {
        // Placeholder: generate fake storage paths
        const jobPhotos = photos.map((file, idx) => ({
 ...
 */
export async function submitReport(instanceId, userId, reportPayload, photoIds = []) {
    try {
        const { data, error } = await supabase
            .from('job_reports')
            .insert({
 ...
 */
export async function approveReport(reportId, approverId) {
    try {
        const { data, error } = await supabase
            .from('job_reports')
            .update({
 ...
 */
export async function rejectReport(reportId, approverId, reason) {
    try {
        const { data, error } = await supabase
            .from('job_reports')
            .update({
 ...
 */
export async function getReportByInstance(instanceId) {
    try {
        const { data, error } = await supabase
            .from('job_reports')
            .select('*')
