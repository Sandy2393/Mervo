...
let serverFlags: FlagDefinition[] = [];

export function loadServerFlags(defs: FlagDefinition[]) {
  serverFlags = defs;
}

export function evaluateFlagForUser(flagKey: string, ctx: UserContext): EvaluationResult {
...
export function batchEvaluate(flagKeys: string[], contexts: UserContext[]): EvaluationResult[] {
