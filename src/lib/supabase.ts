...
const meta: any = (typeof import.meta !== 'undefined' ? (import.meta as any) : {}) as any;
const URL = (meta?.env?.VITE_SUPABASE_URL as string) || ENV.SUPABASE_URL || '';
const ANON = (meta?.env?.VITE_SUPABASE_ANON_KEY as string) || ENV.SUPABASE_ANON_KEY || '';
...
if (ENV.SUPABASE_URL.includes('REPLACE_ME') || ENV.SUPABASE_ANON_KEY.includes('REPLACE_ME')) {
  console.error(
    '[SUPABASE] Configuration incomplete. Placeholder values detected in env.ts. ' +
