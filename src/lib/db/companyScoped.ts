 ...
 */
export function getScopedClient(companyId: string | null) {
  if (!companyId) throw new Error('companyId is required to get scoped client');

  return {
    from: (table: string) => {
      // returns a supabase query with .eq('company_id', companyId) applied
      const q = supabase.from(table as any).select('*').eq('company_id', companyId);
