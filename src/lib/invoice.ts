 ...
 */
export async function buildInvoiceFromInstances(company_id: string, instanceIds: string[], meta: { currency?: string; periodStart?: string; periodEnd?: string } = {}): Promise<Invoice> {
  ...
  const lineItems: InvoiceLineItem[] = instances.map((inst: any) => {
 ...
 */
export function invoiceNumberGenerator(companyTag: string, date: Date = new Date(), seq: number = 1): string {
