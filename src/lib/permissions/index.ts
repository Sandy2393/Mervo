 ...
 */
export function hasPermission(user: UserProfile | null, companyId: string, required: Permission): boolean {
  ...
  const order = { none: 0, view: 1, edit: 2 } as Record<Permission, number>;
  return order[p as Permission] >= order[required];
}
 ...
 */
export function canPerform(user: UserProfile | null, companyId: string, minRoleLevel: number): boolean {
  if (!user || !companyId) return false;
  ...
  const rl = (c as any).role_level;
  if (typeof rl !== 'number') return false;
