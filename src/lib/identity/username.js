...
export function validateUsername(username) {
    if (!username || typeof username !== 'string')
        return { valid: false, reason: 'username_missing' };
    if (username.includes('@'))
        return { valid: false, reason: "username_must_not_contain_at_sign" };
    if (!USERNAME_REGEX.test(username))
        return { valid: false, reason: 'username_invalid_chars' };
...
export function buildMasterId(username, appTag) {
    const clean = normalizeUsername(username);
    const tag = (appTag || '').trim().toLowerCase();
    if (!tag)
        throw new Error('appTag is required to build master id');
    return `${clean}@${tag}`;
}
export function buildCompanyAlias(username, companyTag) {
    const clean = normalizeUsername(username);
    const tag = (companyTag || '').trim().toLowerCase();
    if (!tag)
        throw new Error('companyTag is required to build company alias');
    return `${clean}@${tag}`;
}
// Export a helper that normalizes whether a provided value is bare username or masterId
export function ensureMasterId(candidate, appTag) {
    if (!candidate)
        return '';
