...
import { runHardDelete } from "../../retention/executor";
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!confirm) {
      return res.status(400).json({ error: "confirm=true required for hard delete" });
    }
    ...
    const result = await runHardDelete(String(company_id), settings, { confirm, initiated_by: user_id });
